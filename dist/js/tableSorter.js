export class tableSorter {
	constructor(obj){
		...
		document.querySelector(obj.target).appendChild(wrapper)
		...
		this.result = this.obj.dataSet.filter(function(e){
			...
			Object.keys(conditions).forEach(function(c,v){
				...
				if( cellIsNumber === true 
					...
					if(cellIsNumber === false){
						status_.push( e[cols[v]].toLowerCase().includes(value)	)
					}
					else{
						const toString = String(e[cols[v]])
						status_.push(  toString.includes( value ) )
					}
				}
			})		
	...
	drawRows(e){
		...
		filteredData.forEach(function(row,key){
			const tr = document.createElement("tr")
			if(obj.striped=== true){
				tr.classList.add( key % 2 == 0 ? "even" : "odd")
			}
